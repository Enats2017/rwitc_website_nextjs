<?php
/* =====================================================================
   users.php — FINAL, DB-wired version.

   Actual schema (as per your phpMyAdmin structure):
     users (
   ...
   Notes:
   - No separate "username" column in your table -> email is used as the
     login identity everywhere in this UI.
   - No "status" column -> "Active/Disabled" pill is driven by `verified`
     (Y = Active, N = Disabled).
   - No "date_updated" column -> table shows "Added" (created) and
     "Last Login" (last_login) instead of a fake "updated" date.
   - Group name is pulled live via JOIN with user_group (user_group_id).

   NEW IN THIS VERSION:
   - Server-side pagination, 20 users per page.
   - List is ordered by id DESC, so a newly added user always shows up
     on top (page 1), not at the bottom.
   - Status filter (All / Active / Disabled) — server side.
   - Search box now matches firstname, lastname, and email as a
     substring (LIKE '%term%'), so searching "r" will match "Rohan"
     (because of the "h"... wait, no — it matches because LIKE looks
     for the letter anywhere in the string, not just at the start).
     Search + filter + pagination all work together correctly.

   IMPORTANT - password column is only VARCHAR(45).
   password_hash() (bcrypt) produces a 60-character string, which WILL
   get truncated by MySQL and break login. Run this once before using
   this page:

       ALTER TABLE users MODIFY password VARCHAR(255) NOT NULL;

   The code below assumes you've widened the column. If you haven't,
   widen it first - otherwise every saved password will be silently
   cut off and login will never work.
   ===================================================================== */
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['bulk_ids']) && isset($_POST['bulk_delete'])) {
    ...
    if (!empty($ids)) {
        ...
        header("Location: users.php?msg=error&err=" . urlencode("No users selected."));
        exit;
    }
}
            ...
            if ($dup) {
                ...
                if ($edit_id) {
                    $sql = "UPDATE users SET "
                         ...
                         . "verified='" . $verified . "'";
                    if ($password !== '') {
                        $sql .= ", password='" . $db->escape(password_hash($password, PASSWORD_DEFAULT)) . "'";
                    }
...
if ($action == 'list') {
    ...
    $active_count   = count(array_filter($all_verified, function($u){ return $u['verified'] == 'Y'; }));
...
// Builds a users.php?... link that keeps the current search/filter/page state
...
<head>
<meta charset="utf-8">
<title>Users - RWITC Access Control</title>
...
.stat-strip{display:flex;gap:0;background:var(--surface);border:1px solid var(--line);border-radius:var(--radius);box-shadow:var(--shadow);margin-bottom:24px;overflow:hidden;}
...
.stat.active .num{color:var(--green-600);}
.stat.inactive .num{color:var(--danger);}
...
.status-filter{padding:9px 12px;border:1px solid var(--line);border-radius:8px;font-size:13.5px;font-family:inherit;background:#fff;color:var(--ink-900);outline:none;cursor:pointer;}
.status-filter:focus{border-color:var(--green-600);}
.results-note{font-size:12.5px;color:var(--ink-600);margin-bottom:10px;}
...
.avatar-ring{width:34px;height:34px;border-radius:50%;background:var(--green-700);color:#fff;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;border:2px solid var(--gold-400);flex-shrink:0;}
.user-name{font-weight:600;color:var(--ink-900);}
.user-sub{font-size:12px;color:var(--ink-400);}

.pill-group{display:inline-block;padding:3px 10px;border-radius:20px;font-size:11.5px;font-weight:600;background:#f1f0e8;color:var(--green-900);border:1px solid #e3e1d2;}
.pill-none{color:var(--ink-400);font-size:12.5px;}
...
.page-link{min-width:34px;height:34px;padding:0 10px;display:inline-flex;align-items:center;justify-content:center;border:1px solid var(--line);border-radius:7px;background:#fff;color:var(--ink-700,var(--ink-900));font-size:13px;font-weight:600;}
.page-link:hover{border-color:var(--green-600);color:var(--green-700);}
...
.field input,.field select{width:100%;padding:10px 13px;border:1px solid var(--line);border-radius:8px;font-size:13.5px;font-family:inherit;background:#fff;color:var(--ink-900);outline:none;transition:border-color .15s, box-shadow .15s;}
...
@media (max-width: 720px){
    ...
    tbody tr{border-bottom:1px solid var(--line);padding:14px 16px;position:relative;}
    tbody td{border:0;padding:6px 0;display:flex;justify-content:space-between;align-items:center;gap:10px;}
    tbody td:before{content:attr(data-label);font-size:11px;font-weight:600;color:var(--ink-400);text-transform:uppercase;letter-spacing:0.04em;flex-shrink:0;}
    tbody td.user-td:before{display:none;}
    tbody td.user-td{padding-bottom:10px;}
...
<div class="topbar">
    ...
    <div class="topbar-nav">
        ...
        <a href="userGroup.php">User Groups</a>
    </div>
</div>
...
<div class="page-wrap">
    ...
    <?php if ($action == 'list'): ?>
        ...
        <div class="page-head">
            ...
            <a href="users.php?action=form" class="btn btn-primary"><i class="fa fa-plus"></i> Add User</a>
        </div>
        ...
        <form method="get" action="users.php" id="filter-form" class="toolbar">
                    ...
                    <tr class="user-row">
                        ...
                        <td data-label="">
                            <div class="row-actions">
                                <a href="users.php?action=form&id=<?php echo (int)$u['id']; ?>" class="icon-btn" title="Edit"><i class="fa fa-pencil"></i></a>
                                <a href="users.php?action=delete&id=<?php echo (int)$u['id']; ?>" class="icon-btn danger" title="Delete"
    ...
    <?php else: ?>
        ...
        <div class="page-head">
            ...
            <a href="users.php" class="btn btn-ghost"><i class="fa fa-arrow-left"></i> Back to list</a>
        </div>

        <form method="post" action="users.php?action=form" id="form-user">
