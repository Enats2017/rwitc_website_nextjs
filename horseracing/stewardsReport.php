...
// ===== Next.js API handler (returns raw HTML, no layout) =====
if (isset($_GET['api']) && $_GET['api'] == 'list') {
    header("Access-Control-Allow-Origin: *");
    header("Access-Control-Allow-Methods: GET");
    header("Content-Type: application/json; charset=UTF-8");

    $apiSrObj = new StewardsReport($db);
    $apiList = $apiSrObj->getAllStewardsReports();

    echo json_encode(["success" => true, "data" => $apiList]);
    exit;
}
...
?>
<style type="text/css">
    #leftArea.col-lg-9 .contentTable th.thwhite {
        color: #ffffff !important;
        background: #04160c !important;
    }

    #leftArea.col-lg-9 .contentTable th.thwhite h2 {
        color: #ffffff !important;
        margin: 0;
        font-size: 22px;
    }
...
if ($id > 0) {
    ...
    try {
        ...
        echo "<a href='horseracing/stewardsReport.php'>Back</a><br />";

        //    include_once($base.STEWARDS_REPORT_BASE."/".$reportDet['filename']);  

        if (strpos($reportDet['filename'], 'http') === 0) {
            echo file_get_contents($reportDet['filename']);
        } else {
            include_once($base . STEWARDS_REPORT_BASE . "/" . $reportDet['filename']);
        }
    } catch (Exception $err) {
...
?>
    ...
    <table class="contentTable" style="margin-top:20px;">
        ...
        <tr>
            ...
            <th class="thwhite" colspan="2">
                <h2>STEWARDS NOTICES</h2>
            </th>
        ...
        <?php foreach ($allReports as $report) { ?>
            ...
            <tr>
                ...
                <td><a href='horseracing/stewardsReport.php?id=<?php echo $report['id']; ?>'><?php echo $report['title']; ?></a></td>
