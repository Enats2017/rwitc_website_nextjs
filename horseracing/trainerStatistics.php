...
include_once('../lib/race.class.php');
  ...
  ?>
  <style type="text/css">
  #leftArea.col-lg-9 .contentTable th { text-align: center; }
  </style>
  <?php

  $lastUpdateDate = $raceObj->getMaxDate("RACEDATE","fhorse5");

  $trainerData = $raceObj->getTrainerStats();

  ?>

<h2>Trainer's Statistics as on <?php echo date("d-M-Y",strtotime($lastUpdateDate)); ?></h2>
   ...
   <col width="150" /><col width="93" /><col width="93" /><col width="93" /><col width="93" /><col width="93" /><col width="93" />
          ...
          <?php foreach ($trainerData as $trainer) { ?>

            <tr>

                <th class="alignLeft darkGrey" style="padding-left: 5px; text-align: center;"><?php echo $trainer['NAME']; ?></th>                

                <td><?php echo $trainer['LWIN']; ?></td>

                <td><?php echo $trainer['LSEC']; ?></td>

                <td><?php echo $trainer['LTHI']; ?></td>

                <td><?php echo $trainer['LFOU']; ?></td>

                <td><?php echo $trainer['LMOUNTS']; ?></td>

                <td><?php echo round((($trainer['LWIN']*100) / $trainer['LMOUNTS']),2); ?></td>
