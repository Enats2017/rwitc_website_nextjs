...
if (isAdminlogin()) {
	if ($_SESSION['sponsorManager'] == "Y") { // check login
		...
		if ($q == "delete-sponsor") {
			...
			$sponsorDetails = $rObj->getSponsorById($sponsorID);
			try {
				//unlink(DIR_SPONSOR_UPLOAD."/".$sponsorDetails);   
				$rObj->deleteSponsorByID($sponsorID);
				$_SESSION['sponsor_msg'] = 'Sponsor Deleted Successfully!';
				header("Location: sponsorManager.php");
				exit;
			} catch (Exception $err) {
				$msg = $err->getMessage();
			}
		...
		if (isset($_REQUEST['submit'])) {
			...
			if ($q == "save-data") {
				...
				foreach ($_POST['sponsor_datas'] as $bkey => $value) {
					...
					try {
						$rObj->updateSponsor($id, $title, $link, $sort_order);
					} catch (Exception $err) {
						echo $err->getMessage();
					}
				}
			...
			if (isset($_FILES['file']) && $q == 'image-upload') {
				...
				foreach ($files_arr as $fkey => $fvalue) {
					...
					if ($s3_url != '') {
						$sql = "INSERT INTO `sponsor` SET `title` = '" . $file_title . "', `link` = '', `source` = '" . $s3_url . "', `sort_order` = '" . $cnt . "' ";



						// $sql = "INSERT INTO `sponsor` SET `title` = '".$file_title."', `link` = '', `source` = '".$file."', `sort_order` = '".$cnt."' ";

						$rObj->insertSponsorimage($sql);

						$cnt++;
					}
				}
				$_SESSION['sponsor_msg'] = 'Sponsor Images Uploaded';
				header("Location: sponsorManager.php");
				exit;
			}
		}
		$sponsor_datas = $rObj->getsponsor_datas();
		$sponsor_datas_count = count($sponsor_datas) + 1;
		// echo '<pre>';
...
<style type="text/css">
	...
	.sponsor-upload-table {
		width: 100%;
		border-collapse: collapse;
	}

	.sponsor-upload-table th {
		text-align: left;
		padding: 10px 8px;
		color: #2b332f;
		vertical-align: top;
	...
	.sponsor-upload-table input[type="submit"] {
		background: #0f5c33;
		color: #fff;
		border: none;
		padding: 9px 20px;
	...
	.sponsor-upload-table input[type="submit"]:hover {
		background: #0b3d24;
	}

	.sponsor-upload-table input[type="reset"] {
		background: #fff;
		color: #2b332f;
		border: 1px solid #e2e6e4;
		padding: 9px 20px;
	...
	.sponsor-upload-table input[type="reset"]:hover {
		background: #f5f4ee;
	}
	...
	.sponsor-card .sponsor-card-body {
		padding: 12px 14px;
		display: flex;
		flex-direction: column;
		gap: 10px;
	...
	.sponsor-card .sponsor-card-actions {
		display: flex;
	...
	.sponsor-card .sponsor-card-actions a {
		font-size: 13px;
	...
	.sponsor-save-bar input[type="submit"] {
		background: #0f5c33;
		color: #fff;
		border: none;
		padding: 10px 22px;
	...
	.sponsor-save-bar input[type="submit"]:hover {
		background: #0b3d24;
	}
	...
	@media (max-width: 520px) {
		...
		.sponsor-upload-table,
		.sponsor-upload-table tbody,
		.sponsor-upload-table tr,
		.sponsor-upload-table th,
		.sponsor-upload-table td {
			display: block;
			width: 100% !important;
...
<?php 
if (isset($_SESSION['sponsor_msg'])) {
    $json['success'] = $_SESSION['sponsor_msg'];
    unset($_SESSION['sponsor_msg']);
}
?>
			...
			<tr>
				<td colspan="2">
					...
					<!-- <input type="reset" name="reset" value="Clear" onclick="location.href='turf-console/sponsorManager'" /> -->
					 <input type="reset" name="reset" value="Clear" onclick="location.href='turf-console/sponsorManager.php'" />
				</td>
			</tr>
		</table>
	</form>
