    ...
    if ($_SESSION['dividends'] == "Y") { // check login
                    ...
                    if (!$_FILES['dividendFile']['error']) { // error =0  
                        ...
                        try {
                            ...
                            $result = $s3Client->putObject([
                                'Bucket'      => AWS_BUCKET,
                                'Key'         => 'uploads/Dividends/' . $filename,
                                'SourceFile'  => $_FILES['dividendFile']['tmp_name'],
                                // 'ACL'         => 'public-read',
                        ...
                        if ($s3_url != '') {
                            $id = $dividendObj->insertDividend($date, $centreid, $s3_url);
                            $_SESSION['dividend_msg'] = 'Dividend Uploaded Successfully!';
                            header("Location: dividendsManager.php");
                            exit;
                        }
                    }
                } catch (Exception $err) {
        ...
        if ($q == "delete-dividend") {
            ...
            try {
                ...
                $_SESSION['dividend_msg'] = 'Dividend Deleted Successfully!';
                header("Location: dividendsManager.php");
                exit;
            } catch (Exception $err) {
                $msg = $err->getMessage();
    ...
    <script type="text/javascript">
        function confirmDelete(dividendID) {
            if (confirm ("Are you sure ?")){
                location.href="turf-console/dividendsManager.php?q=delete-dividend&id="+dividendID;
            }
        }
    </script>
  ';
...
<style type="text/css">
    ...
    .dividend-form-table input[type="reset"] {
        background: #0f5c33;
        color: #fff;
        border: none;
        padding: 9px 20px;
    ...
    table.dividends-table th.action-col {
        text-align: right;
        width: 120px;
    }

    table.dividends-table td {
        padding: 14px 20px;
        border-bottom: 1px solid #eef0ee;
        color: #2b332f;
    }

    table.dividends-table tr:last-child td {
        border-bottom: none;
    }

    table.dividends-table tr:nth-child(even) td {
        background: #f7faf8;
    }

    table.dividends-table tr:hover td {
        background: #e6f4ec;
    }

    table.dividends-table td.action-col {
        text-align: right;
        white-space: nowrap;
    }

    table.dividends-table td.action-col a {
        font-size: 13.5px;
        text-decoration: none;
        font-weight: 500;
        color: #c0392b;
    ...
    @media (max-width: 520px) {
        ...
        .dividend-form-table,
        .dividend-form-table tbody,
        .dividend-form-table tr,
        .dividend-form-table th,
        .dividend-form-table td {
            display: block;
            width: 100% !important;
...
<?php
if (isset($_SESSION['dividend_msg'])) {
    $msg = $_SESSION['dividend_msg'];
    unset($_SESSION['dividend_msg']);
}
?>
<?php if (!empty($msg)) { ?>
...
<?php if ($_SESSION['dividends'] == "Y") { ?>
    ...
    <div class="dividends-header">
        <a class="add-dividend-btn" href="turf-console/dividendsManager.php?q=new-dividend"><i class="fas fa-plus"></i> Add New Dividend</a>
        <!--
    ...
    <?php if ($q == "new-dividend" || $q == "edit-dividend") { ?>
        <div class="dividend-form-wrap">
            ...
            <form name="dividendForm" method="post" action="turf-console/dividendsManager.php" enctype="multipart/form-data">
            ...
            <?php if (count($alldividendPageWise) > 0) { ?>
                <?php foreach ($alldividendPageWise as $dividendInfo) { ?>
                    <tr>
                        ...
                        <td class="action-col">
                            <!--  <a href="/turf-console/dividendsManager.php?id=<?php echo $dividendInfo['id']; ?>&q=edit-dividend">Edit</a>         -->

                            <a href="#" onclick="javascript: confirmDelete(<?php echo $dividendInfo['id']; ?>); return false;"><i class="fas fa-trash-alt"></i> Delete</a>
                        </td>
                    </tr>
                <?php } ?>
            <?php } else { ?>
    ...
    <?php if ($totalDividends > 0) {
        $paging->writePagination();
    } ?>
<?php } ?>
<?php
$design->closeDiv();
