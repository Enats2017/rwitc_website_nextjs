	...
	if ($_SESSION['bannerManager'] == "Y") { // check login
		...
		if ($q == "delete-banner") {
			...
			try {
				...
				$_SESSION['banner_msg'] = 'Banner Deleted Successfully!';
		...
		if (isset($_REQUEST['submit'])) {
			...
			if (isset($_FILES['file']) && $q == 'image-upload') {
				...
				foreach ($files_arr as $fkey => $fvalue) {
					...
					if ($s3_url != '') {
						$sql = "INSERT INTO `banner` SET `banner_id` = '1', `title` = '" . $file_title . "', `source` = '" . $s3_url . "', `sort_order` = '" . $cnt . "' ";
				...
				$_SESSION['banner_msg'] = 'Banner Images Uploaded';
				header("Location: bannerManager.php");
				exit;
			}
...
<style type="text/css">
	...
	.banner-upload-table input[type="submit"] {
		background: #0f5c33;
		color: #fff;
		border: none;
		padding: 9px 20px;
	...
	.banner-upload-table input[type="submit"]:hover {
		background: #0b3d24;
	}

	.banner-upload-table input[type="reset"] {
		background: #fff;
		color: #2b332f;
		border: 1px solid #e2e6e4;
		padding: 9px 20px;
	...
	.banner-upload-table input[type="reset"]:hover {
		background: #f5f4ee;
	}
	...
	.banner-card .banner-card-actions {
		display: flex;
	...
	.banner-card .banner-card-actions a {
		font-size: 13px;
	...
	.banner-save-bar input[type="submit"] {
		background: #0f5c33;
		color: #fff;
		border: none;
		padding: 10px 22px;
	...
	.banner-save-bar input[type="submit"]:hover {
		background: #0b3d24;
	}
	...
	@media (max-width: 520px) {
		...
		.banner-upload-table,
		.banner-upload-table tbody,
		.banner-upload-table tr,
		.banner-upload-table th,
		.banner-upload-table td {
			display: block;
			width: 100% !important;
...
if (isset($_SESSION['banner_msg'])) {
	$json['success'] = $_SESSION['banner_msg'];
...
<!--
	<div class="submenu">
	  	...
	  	<a href="turf-console/bannerManager.php">Banner Manager</a>
			...
			<?php foreach ($banner_datas as $bkey => $bvalue) { ?>
				...
				<div class="banner-card">
					<div class="thumb-wrap">
						...
						<img src="<?php echo (strpos($bvalue['source'], 'http') === 0) ? $bvalue['source'] : HTTP_BANNER_UPLOAD . '/' . $bvalue['source']; ?>" alt="" />
