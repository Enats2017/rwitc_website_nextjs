...
$Itemapi = new Itemapi();
$value = '';
if(isset($_GET['type']) && $_GET['type'] == 'race_result') {
...
class Itemapi {
	...
	public function insertAcceptanceapp($datas = array(), $handle){
		...
		if(!empty($datas)){
			...
			$insert_query = $this->query("INSERT INTO `erp_raceresult_app` SET `race_date` = '".$this->escape($race_date,$this->conn)."', `data` = '".$this->escape($data,$this->conn)."', `raceresult_date` = '".date('Y-m-d')."' ", $this->conn);
		}       
