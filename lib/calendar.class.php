...
Class Calendar {
    ...
    // ---- New METHOD ADD ----
    function getexistdata($centreid, $date) {
        return $this->db->getSingleRowAssoc(self::sqlGetExistData($centreid, $date));
    }

    private static function sqlGetExistData($centreid, $date) {
        return "SELECT id, racedate, centreid 
                FROM racing_calendar
