...
class Itemapi {
    ...
    public function insertAcceptanceapp($datas = array(), $handle){
        ...
        if(!empty($datas)){
            ...
            $this->query("DELETE FROM erp_raceresult_app_2 WHERE `race_date` = '".$this->escape($race_date,$this->conn)."'",$this->conn);
            $insert_query = $this->query("INSERT INTO `erp_raceresult_app_2` SET  `race_date` = '".$this->escape($race_date,$this->conn)."', `data` = '".$this->escape($data,$this->conn)."'", $this->conn);
        }       
