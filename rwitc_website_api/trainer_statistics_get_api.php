...
$security = new ApiSecurity($handle, [
    ...
    "api_tag"        => "trainer_statistics_get"
]);
...
try {
    ...
    // -------------------------------------------------------
    // 1) Pehle run_races/Trainer_statistics.html (old page jaisa)
    //    Is file me heading + table dono already hote hain,
    //    isliye as_on null bhejte hain.
    // -------------------------------------------------------
    $localFile = rtrim((string) RUN_RACES_LOCAL_PATH, "/\\") . "/Trainer_statistics.html";
    ...
    if (is_file($localFile)) {
        ...
        // ---------------------------------------------------
        // 2) Fallback: DB (old date1 + trainerData query)
        // ---------------------------------------------------

        // MAX(RACEDATE) from fhorse5
        $asOn = null;
        ...
        // statt table query (same as old page)
        $result = $conn->query("
    ...
    $security->logLine("TRAINER_STATISTICS_API_ERROR | " . $error->getMessage());
