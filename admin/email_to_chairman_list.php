...
//   $user = "rwitc_erp";
  ...
  session_start(); 
    ...
    if(isset($_GET['type']) && $_GET['type'] == 'delete'){
        $id=$_GET['id'];
        $q="DELETE FROM `email_to_chairman` WHERE id='".$id."'";
        mysqli_query($conn,$q);
    }


    $sql = "SELECT * FROM `email_to_chairman` WHERE 1=1";
    ...
    $totalChairmanMails = count($result2Full);
    $paging = new Pagination($pageno,15,$totalChairmanMails);
...
<h1 class="feedback-title"><i class="fas fa-envelope-open-text"></i> Email to Chairman</h1>
...
<table class="feedback-table">
  ...
  <tbody>
    <?php if(empty($result2)){ ?>
        <tr><td colspan="6" class="feedback-empty">No records found.</td></tr>
    <?php } ?>
    <?php if($result2 != ""){?>
    ...
    <tr>
      ...
      <td><a href="admin/email_to_chairman_list.php?type=delete&id=<?php echo $res3['id'];  ?>"><button class="feedback-action-btn delete"><i class="fas fa-trash-alt"></i> Delete</button></a></td>
   
      <td><a href="admin/email_to_chairman_form.php?type1=edit&id=<?php echo $res3['id'];?>"><button class="feedback-action-btn edit"><i class="fas fa-edit"></i> Edit</button></a></td>
    </tr>
   <?php }} ?>
  </tbody>
</table>
...
<script type="text/javascript">
    function delete_feedback(id){ 
        console.log(id);
        $.ajax( {
        url : 'admin/email_to_chairman_list.php?type=delete&id='+id,
        type: 'GET',
...
$('#button-filter').on('click', function() {
   ...
   var url = 'admin/email_to_chairman_list.php?type=autocomplete';
...
$('input[name=\'filter_name\']').autocomplete({
    'source': function(request, response) {
        $.ajax({
            url: 'admin/feedback_query.php?type=filter_name&method=feedback1&filter_name=' + (request.term),
            dataType: 'json',
...
$('input[name=\'filter_email\']').autocomplete({
    'source': function(request, response) {
        $.ajax({
             url: 'admin/feedback_query.php?type=filter_email&method=feedback1filter_email=' + (request.term),
            dataType: 'json',
            success: function(json) {
                response($.map(json, function(item) {
                    return {
